...
trait CanHaveScratchpad extends HasHellaCache with HasICacheFrontend {
  ...
  val busErrorUnit = tileParams.core.busErrorUnitAddr map { a =>
    val beu = LazyModule(new BusErrorUnit(new L1BusErrors, a))
    intOutputNode := beu.intNode
    beu
  }
...
trait CanHaveScratchpadBundle extends HasHellaCacheBundle with HasICacheFrontendBundle {
  ...
  val intOutput = outer.intOutputNode.bundleOut
}
